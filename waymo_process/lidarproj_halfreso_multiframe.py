...
def projpc(data,frame_points,intid2inboxpoints,waymoid2intid,idx,output_path,resomult=0.5,multiframe_num=2,proj_territory_size=4):
    ...
    for objpoints in objs_points:
        all_points.append(objpoints[:,:3])
        all_points_colors.append(objpoints[:,::-1][:,:3])

    all_points.append(merged_lidar_points[:,:3])
    all_points_colors.append(merged_lidar_points[:,::-1][:,:3])

    ply_folder = os.path.join(output_path, "pts")
    os.makedirs(ply_folder, exist_ok=True)
    ply_filename = os.path.join(ply_folder, "pts.las")
    save_las(np.concatenate(all_points, axis=0), np.concatenate(all_points_colors, axis=0), ply_filename)
